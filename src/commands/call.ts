...
Examples:
  ...
  .action(
    async (
      tool: string,
      commandName: string,
      options: { json?: boolean; agent?: boolean; verbose?: boolean },
      cmd: Command,
    ) => {
      ...
      if (!toolCommand) {
        ...
        console.error(
          chalk.gray(`Available commands: ${manifest.commands.map((c) => c.name).join(', ')}`),
        );
        process.exitCode = 1;
        return;
      }
      ...
      if (isVerbose) {
        console.error(
          chalk.gray(`[verbose] ${toolCommand.method} ${manifest.base_url}${toolCommand.path}`),
        );
